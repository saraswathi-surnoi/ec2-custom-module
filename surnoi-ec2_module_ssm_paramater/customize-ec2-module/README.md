...
|----------------|-------------|
| `aws_instance` | Creates EC2 instances based on `instances` map |
| `aws_route53_record` | Optionally creates DNS records for instances |
...
|------|------|-----------|----------|--------------|
| `project_name` | `string` | ✅ | n/a | Project name (used for tagging and DNS naming) |
| `environment` | `string` | ✅ | n/a | Deployment environment (`dev`, `qa`, `prod`) |
| `ami_id` | `string` | ✅ | n/a | AMI ID to use for EC2 instances |
| `subnet_id` | `string` | ✅ | n/a | Subnet ID for EC2 deployment |
| `key_pair_name` | `string` | ✅ | n/a | Existing AWS key pair name |
| `instances` | `map(object)` | ✅ | n/a | Map of EC2 instance configurations |
| `sg_map` | `map(string)` | ✅ | n/a | Map of Security Group IDs from `sg-module` |
| `common_tags` | `map(string)` | ✅ | `{}` | Common tags applied to all resources |
| `create_dns` | `bool` | ❌ | `false` | Whether to create Route53 DNS records |
| `route53_zone_id` | `string` | ❌ | `""` | Hosted Zone ID for Route53 |
